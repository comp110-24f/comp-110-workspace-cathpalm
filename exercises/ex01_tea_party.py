...
def main_planner(guests: int) -> None:
    ...
    print("Treats: " + str(treats(attendees=guests)))
    print(
        "Cost: $"
        + str(
            cost(
                tea_count=tea_bags(people=guests), treat_count=treats(attendees=guests)
            )
        )
    )
...
def treats(attendees: int) -> int:
    """Determines the number of treats needed"""
    return int(tea_bags(people=attendees) * 1.5)
